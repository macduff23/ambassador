...
func TestGetExistingInstallationHelm(t *testing.T) {
	// contents of a `kubectl get deployments -n ambassador -l app.kubernetes.io/managed-by=edgectl -o json` (with some parts removed)
	// when it has been deployed with Helm
	k := SimpleKubectl{
		// force an output, so no real `kubectl` is run
		output: `
{
